...
MART_NAME = "daily_segment_performance_mart"
...
def record_batch_failure(context):
    ...
    source_count_sql = f"""
        SELECT uniqExact(tuple(segment_id, campaign_id))
        FROM settlement_analytics.segment_performance_metrics
    ...
    mart_count_sql = f"""
        SELECT count()
        FROM settlement_analytics.daily_segment_performance_mart
...
def assert_aggregate_matches(source_row, mart_row, report_date, batch_run_id):
    ...
    for index, metric_name in enumerate(metric_names):
        if source_row[index] != mart_row[index]:
            raise ValueError(
                f"Segment mart validation failed. "
...
def rebuild_daily_segment_mart(**context):
    ...
    source_aggregate_sql = f"""
        SELECT
            uniqExact(tuple(segment_id, campaign_id)) AS segment_campaign_count,
    ...
    insert_sql = f"""
        INSERT INTO settlement_analytics.daily_segment_performance_mart
        (
            report_date,
            segment_id,
            campaign_id,
        ...
        (
            ...
            WHERE toDate(window_start) = '{report_date}'
              AND partial = false
            GROUP BY report_date, segment_id, campaign_id
        )
    """

    mart_aggregate_sql = f"""
        SELECT
            count() AS segment_campaign_count,
            sum(impression_count) AS impression_count,
            sum(click_count) AS click_count,
            sum(conversion_count) AS conversion_count,
            sum(total_deducted_amount) AS total_deducted_amount,
    ...
    if source_count == 0:
        ...
        print(f"No source segment metrics for report_date={report_date}. skip mart build.")
        return
...
with DAG(
    dag_id="daily_segment_performance_mart",
    description="Build daily segment performance mart from minute-level segment metrics",
    start_date=pendulum.datetime(2026, 1, 1, tz="UTC"),
    schedule="@daily",
    catchup=False,
    default_args=default_args,
    tags=["settlement", "segment", "mart"],
) as dag:
    rebuild_daily_segment_mart_task = PythonOperator(
        task_id="rebuild_daily_segment_performance_mart",
        python_callable=rebuild_daily_segment_mart,
